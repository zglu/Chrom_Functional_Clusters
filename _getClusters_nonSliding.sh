...
#1 gene-chr-start.txt	gene chromosome start_coord (only for chromosomes), sep by " "
...
#4 chr-length.txt	chromosome length, sep by " "
# SM_V7_1 88881357
# SM_V7_2 48130368
...
# make Fisher table
cd splitChr/
for i in *.genes; do sort $i | join - ../gene-func.txt | sed 's/,/ /g'| awk -v OFS='\t' '{for (i=2;i<=NF;i++) print $1,$i}' | awk '{print $2}'| sort | uniq -c | awk '{print $2 " " $1}'|join - ../func-reference.txt | join ../func-names.txt - | awk '{print FILENAME $1 ":" $2 " " $3 " " $4 " " "'$BLOCK'"-$3 " " "'$TOTAL'"-$4}' > $i-sum.txt; done

# print file name and columns
for i in *sum.txt; do awk '{print FILENAME $0}' $i | sed 's/\.genes-sum\.txt/ /' > $i-table.txt; done
